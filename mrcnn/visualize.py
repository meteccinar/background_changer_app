...
def blur_image(image, masked_image):
    ...
    blurred_image = cv2.blur(blurred_image,(15,15))
...
def change_background(image,newBg, masked_image):
...
def display_blurred(image, boxes, masks, class_ids, class_names,
                            scores=None , figsize=(16,16), ax=None,name="output.png"):
...
def display_newBG(image, newBg,boxes, masks, class_ids, class_names,
                            scores=None , figsize=(16,16), ax=None,name="output.png"):
        ...
        newBG = change_background(image,newBg , masked_image)

        plt.imsave(name, newBG.astype(np.uint8))
        return newBG,masked_image;
...
def display_instances(image, boxes, masks, class_ids, class_names,
    ...
    for i in range(N):
        ...
        if show_mask :
            ...
            plt.imsave(filename,masked_image.astype(np.uint8))
        ...
        ax.imshow(padded_mask)
        contours = find_contours(padded_mask, 0.5)
        for verts in contours:
            # Subtract the padding and flip (y, x) to (x, y)
            verts = np.fliplr(verts) - 1
            p = Polygon(verts, facecolor="white", edgecolor="white")
            ax.add_patch(p)
